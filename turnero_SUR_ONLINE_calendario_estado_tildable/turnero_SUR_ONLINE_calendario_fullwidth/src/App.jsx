...
export default function App() {
  ...
  const actualizarEstadoTurno = async (id, nuevoEstado) => {
    ...
    const estadoNuevo = etiquetaEstado(nuevoEstado);

    setTurnos(prev => prev.map(t =>
      String(t.id) === String(id) ? { ...t, estado: estadoNuevo } : t
    ));

    if (String(turnoSeleccionado?.id) === String(id)) {
      setTurnoSeleccionado({ ...turno, estado: estadoNuevo });
    }

    if (supabase) {
      const { error } = await supabase.from("turnos").update({ estado: estadoNuevo }).eq("id", id);
      if (error) {
        alert("No se pudo actualizar el estado del turno. Se vuelve al estado anterior.");
        setTurnos(prev => prev.map(t =>
          String(t.id) === String(id) ? { ...t, estado: estadoAnterior } : t
        ));
        if (String(turnoSeleccionado?.id) === String(id)) {
  ...
  const BotonesEstado = ({ turno, compact = false }) => {
    const actual = normalizarEstado(turno.estado);
    const opciones = [
      ["asistio", "✓ Asistió"],
      ["aviso", "Avisó"],
      ["no-aviso", "No avisó"]
    ...
    return (
      <div className={`status-actions ${compact ? "compact" : ""}`}>
        {opciones.map(([key, label]) => (
          <button
            key={key}
            type="button"
            draggable={false}
            className={`${key} ${actual === key ? "active" : ""}`}
            onMouseDown={e => e.stopPropagation()}
            onPointerDown={e => e.stopPropagation()}
            onClick={e => {
              e.preventDefault();
              e.stopPropagation();
              actualizarEstadoTurno(turno.id, key);
            }}
          >
            {label}
          </button>
